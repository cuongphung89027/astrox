...
import { KD_METHODS, castHexagram, castCoins, castDigits, castTime, coinValue, throwCoins, createKdHistory, replayKdHistory, readKdHistory, pushKdHistory, removeKdHistory } from "@/lib/kinhdich";
...
export function KinhDichClient() {
  ...
  function stop(){if(timer.current)clearTimeout(timer.current);timer.current=null;sequence.current=null;setPhase("idle");setRound(0);setFaces([]);setValues([]);}
  useEffect(()=>()=>{if(timer.current)clearTimeout(timer.current);sequence.current=null;},[]);
  function reveal(){const all=sequence.current;if(!all)return;if(timer.current)clearTimeout(timer.current);sequence.current=null;setPhase("idle");finish(castCoins(all.map(coinValue),all));}
  function start(){
    if(sequence.current)return;
    const all=Array.from({length:6},()=>throwCoins());sequence.current=all;setFaces([]);setValues([]);setError("");
    if(window.matchMedia("(prefers-reduced-motion: reduce)").matches||document.documentElement.dataset.motion==="reduced"){reveal();return;}
    coinScene.current?.scrollIntoView({block:"center",behavior:"smooth"});
    const next=(i:number)=>{if(sequence.current!==all)return;setRound(i+1);setPhase("tossing");
      timer.current=setTimeout(()=>{if(sequence.current!==all)return;setFaces(all.slice(0,i+1));setValues(all.slice(0,i+1).map(coinValue));setPhase("settled");
        timer.current=setTimeout(()=>{if(sequence.current!==all)return;if(i===5)reveal();else next(i+1);},i===5?500:260);
      },1000);
    };next(0);
  }
  ...
  function reset(){stop();setCast(null);setValues([]);setFaces([]);setManualValues(["","","","","",""]);setError("");}
  return <section className={styles.page}>
    ...
    {!cast?<div className={styles.setup}>
      <div className={styles.intro}><span className={styles.eyebrow}>KINH DỊCH</span><h2>Một câu hỏi.<br/>Nhiều cách tìm lời đáp.</h2>
        {method === "coins" ? <div ref={coinScene} className={styles.coinScene}>
          <div className={styles.coinOrbits} aria-hidden="true"><i/><i/><i/></div>
          <div className={styles.threeCoins} aria-label="Ba đồng xu" data-phase={phase}>
            {[0,1,2].map(i=>{const face=!manual?faces.at(-1)?.[i]:undefined;return <div className={styles.coinSlot} key={i}>
              <div key={round} className={styles.goldCoin} data-phase={phase} data-coin={i}>
                <svg viewBox="0 0 100 100" aria-hidden="true">
                  <defs><radialGradient id={`${coinId}-gold-${i}`} cx="30%" cy="20%" r="85%"><stop stopColor="#fff0b6"/><stop offset=".55" stopColor="#dfbd6e"/><stop offset="1" stopColor="#a88035"/></radialGradient></defs>
                  <path fill={`url(#${coinId}-gold-${i})`} fillRule="evenodd" d="M50 2a48 48 0 1 1 0 96a48 48 0 1 1 0-96ZM39 39v22h22V39Z"/>
                  ...
                  <path d="M46 21h8M46 25h8M46 75h8M46 79h8M21 46v8M25 46v8M75 46v8M79 46v8" stroke="#a5823e" strokeWidth="2" strokeLinecap="round"/>
                </svg>
              </div>
              <span className={styles.coinCaption}>{phase==="tossing"?"Đang gieo":face===undefined?`Xu ${i+1}`:face?"Ngửa · 3":"Sấp · 2"}</span>
            </div>;})}
          </div>
          <div className={styles.linePreview} aria-hidden="true">{[5,4,3,2,1,0].map(i=>{const v=manual?Number(manualValues[i]):values[i];return <div key={i} data-ready={!!v} data-yang={!!v&&v%2===1} data-moving={v===6||v===9}><i/><i/><span>{i+1}</span></div>;})}</div>
          <p className={styles.throwStatus} role="status">{rolling?`Đang gieo lượt ${round} / 6`:manual?"Nhập kết quả của bạn":""}</p>
          {!manual&&<div className={styles.coinAction}>
            <button className={styles.primary} onClick={start} disabled={rolling}>{rolling?`Đang gieo ${round}/6…`:"Gieo quẻ"}<span aria-hidden="true">↗</span></button>
            {rolling&&<div className={styles.ritualActions}><button onClick={stop}>Dừng gieo</button><button onClick={reveal}>Hiện quẻ ngay</button></div>}
          </div>}
        </div> : <DivinationTube/>}</div>
      <div className={styles.inputPanel}>
        <label htmlFor="kd-method">Cách lập quẻ</label>
        <select id="kd-method" className={styles.methodSelect} disabled={rolling} value={method} onChange={e=>{setMethod(e.target.value as KdMethod);setError("");setDigits("");}}>{Object.entries(KD_METHODS).map(([id,label])=><option key={id} value={id}>{label}</option>)}</select>
        <label htmlFor="kd-question">Điều bạn đang băn khoăn <span>Tùy chọn</span></label>
        <textarea id="kd-question" disabled={rolling} rows={2} maxLength={200} value={question} onChange={e=>setQuestion(e.target.value)} placeholder="Viết điều bạn muốn hỏi…"/>
        {method==="coins"&&<div className={styles.coinSetup}>
          <label className={styles.checkLabel}><input type="checkbox" disabled={rolling} checked={manual} onChange={e=>setManual(e.target.checked)}/> Nhập kết quả gieo xu thật</label>
          {manual?<div className={styles.coinGrid}>{manualValues.map((v,i)=><label key={i}>Hào {i+1} {i===0?"(dưới)":i===5?"(trên)":""}<select aria-label={`Giá trị hào ${i+1}`} value={v} onChange={e=>setManualValues(a=>a.map((x,j)=>j===i?e.target.value:x))}><option value="">Chọn</option>{[6,7,8,9].map(x=><option key={x} value={x}>{x} · {x===6?"Âm động":x===7?"Dương tĩnh":x===8?"Âm tĩnh":"Dương động"}</option>)}</select></label>)}</div>:<>
            ...
            {!rolling&&values.length>0&&<button className={styles.manualToggle} onClick={()=>{setValues([]);setFaces([]);}}>Hủy và gieo lại</button>}
          </>}
        </div>}
        ...
        {["serial","phone","digits"].includes(method)&&<div key={method} className={styles.digitInput}><label htmlFor="kd-digits">{KD_METHODS[method]}</label><input id="kd-digits" value={digits} maxLength={48} autoComplete="off" inputMode={method==="serial"?"text":"tel"} onChange={e=>setDigits(e.target.value)} placeholder={method==="serial"?"AB00123456":method==="phone"?"0912 345 678":"001234"}/></div>}
        {error&&<p role="alert" className={styles.formError}>{error}</p>}
        {(method!=="coins"||manual)&&<button className={styles.primary} onClick={submit} disabled={method==="coins"&&(manual?manualValues.some(v=>!v):values.length!==6)}>Lập quẻ <span>↗</span></button>}
      </div>
