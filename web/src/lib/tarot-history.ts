 ...
 */
import { cacheFingerprint, getState, subscribe } from "./state";
...
function storedEntries(): TarotHistoryEntry[] {
  try {
    const parsed: unknown = JSON.parse(localStorage.getItem(TAROT_HISTORY_KEY) || "[]");
...
export function pushTarotHistory(entry: TarotHistoryEntry): TarotHistoryEntry[] {
  ...
  try {
    localStorage.setItem(TAROT_HISTORY_KEY, JSON.stringify([next, ...stored]));
...
export function removeTarotHistory(id: string): TarotHistoryEntry[] {
  ...
  localStorage.setItem(DELETED_KEY, JSON.stringify(deleted));
  localStorage.setItem(TAROT_HISTORY_KEY, JSON.stringify(storedEntries().filter(e =>
...
export function subscribeTarotHistory(listener: () => void): () => void {
  ...
  const onStorage = (event: StorageEvent) => { if (event.key === TAROT_HISTORY_KEY || event.key === DELETED_KEY) listener(); };
