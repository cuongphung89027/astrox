...
try{
 await page.goto(base+'/hoso?section=earn');await page.getByRole('button',{name:'Điểm danh ngay'}).waitFor();
 assert.ok(page.url().includes('section=earn'));assert.equal(sdkLoads,0);
 assert.equal(await page.getByRole('heading',{name:'Kiếm thêm Point'}).evaluate(el=>getComputedStyle(el.parentElement).opacity),'1','Earn page must be painted, not only present in DOM');
 await page.getByRole('button',{name:'Điểm danh ngay'}).click();await page.getByRole('button',{name:'Đã điểm danh ✓'}).waitFor();assert.equal(points,12);assert.equal(await page.getByRole('button',{name:'Đã điểm danh ✓'}).isDisabled(),true);
 await page.getByRole('button',{name:'Sao chép link mời'}).click();await page.waitForTimeout(150);assert.ok((await page.evaluate(()=>navigator.clipboard.readText())).endsWith('/?ref=ABC123'));
 for(const width of [360,390,1440]){await page.setViewportSize({width,height:900});await page.getByRole('heading',{name:'Kiếm thêm Point'}).scrollIntoViewIfNeeded();assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));await page.screenshot({path:path.join(artifacts,`earn-${width}.png`),fullPage:true});}
 await page.getByRole('button',{name:'Xem & nhận Point'}).click();assert.equal(sdkLoads,0);assert.equal(starts,0);await page.getByRole('button',{name:'Để sau'}).click();assert.equal(starts,0);assert.equal(sdkLoads,0);
 await page.getByRole('button',{name:'Xem & nhận Point'}).click();await page.getByRole('button',{name:'Đồng ý xem'}).click();await page.waitForFunction(()=>Boolean(window.qaAdEvent));await page.waitForTimeout(200);await page.evaluate(()=>window.qaAdEvent('rewardedSlotClosed'));await page.getByRole('button',{name:'Xem & nhận Point'}).waitFor();assert.equal(grants,0);assert.equal(points,12);
 await page.getByRole('button',{name:'Xem & nhận Point'}).click();await page.getByRole('button',{name:'Đồng ý xem'}).click();await page.waitForTimeout(300);await page.evaluate(()=>{window.qaAdEvent('rewardedSlotGranted');window.qaAdEvent('rewardedSlotGranted');window.qaAdEvent('rewardedSlotClosed');});await page.getByText('Đã nhận +5 Point từ quảng cáo.',{exact:true}).waitFor();assert.equal(grants,1);assert.equal(points,17);
 await page.goto(base+'/hoso?section=points');await page.getByText('Thưởng xem quảng cáo',{exact:true}).waitFor();assert.deepEqual(errors,[]);
 console.log('PASS 360/390/1440 earn page, check-in, referral copy, ad opt-in/cancel/duplicate grant, ledger reason; mocked APIs + SDK, no real provider operation.');
