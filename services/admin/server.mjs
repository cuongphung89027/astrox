...
async function backendReady(env){try{if(!env.ASTROX_BACKEND)return false;const r=await env.ASTROX_BACKEND.fetch(new Request('https://astrox-internal/internal/admin/capabilities',{signal:AbortSignal.timeout(5000)}));return r.ok&&(await r.json()).configVersioned===true}catch{return false}}
async function activationErrors(env,c){const errors=[];if(!await backendReady(env)&&(c.billing.enabled||c.rewards.enabled||c.rewards.ads.enabled||c.integrations.payos.enabled||c.integrations.zalo.enabled||c.integrations.wallet.enabled||c.billing.services.some(s=>s.status==='paid')))errors.push({path:'integrations.wallet',message:'Chưa nối backend ví và xác thực. Không thể kích hoạt chức năng cần backend.'});const refs=c.ai.enabled?c.ai.providers.filter(p=>p.enabled).map(p=>p.secretRef):[];if(c.integrations.payos.enabled)refs.push('payos:apiKey','payos:checksumKey');if(c.integrations.zalo.enabled)refs.push('zalo:appSecret');for(const ref of refs)if(!await readSecret(env,ref))errors.push({path:ref,message:'Chưa lưu khóa kết nối.'});return errors;}
...
export async function handleAdmin(request,env){try{
 ...
 if(path==='config'&&method==='GET'){const s=await state(env),p=await readPublished(env),secrets=await sql(env,'SELECT ref FROM admin_secrets ORDER BY ref').all();return json({draft:JSON.parse(s.draft),revision:s.revision,published:p?.config||null,publishedRevision:p?.revision||null,secrets:secrets.results.map(r=>r.ref),integration:{wallet:await backendReady(env)}});}
