...
const fail = (code, status, attempts) => {
  throw new RuntimeError(code, status, attempts);
};
function safeUrl(value) {
  try {
    ...
    if (
      ...
      ['.local', '.localhost', '.internal', '.test', '.invalid', '.example'].some(s => h.endsWith(s))
    )
      return null;
    return u;
  } catch {
...
async function readJson(response, maxBytes = 1048576) {
  ...
  try {
    while (true) {
      ...
      if (size > maxBytes) fail('RESPONSE_TOO_LARGE', 502);
      chunks.push(value);
    }
    ...
    try {
      ...
      fail('INVALID_PROVIDER_RESPONSE', 502);
    }
  } finally {
...
function normalize(data, protocol, model, attempts) {
  ...
  const usage = {};
  for (const field of ['input_tokens', 'output_tokens', 'prompt_tokens', 'completion_tokens', 'total_tokens'])
  ...
  if (protocol === 'responses') {
    if (
      ...
      data.output.some(
        o =>
          !o ||
          typeof o !== 'object' ||
          (o.type === 'message' && (!Array.isArray(o.content) || o.content.some(c => !c || typeof c !== 'object'))),
      )
    )
      fail('INVALID_PROVIDER_RESPONSE', 502, attempts);
    ...
    if (
      ...
      content.some(c => c.type !== 'output_text')
    )
      fail('INVALID_PROVIDER_RESPONSE', 502, attempts);
    const text = content
      .map(c => c.text)
  ...
  if (protocol === 'anthropic') {
    ...
    if (
      !['end_turn', 'max_tokens', 'stop_sequence'].includes(data.stop_reason) ||
      !Array.isArray(data.content) ||
      !data.content.length ||
      data.content.some(c => !c || c.type !== 'text' || typeof c.text !== 'string')
    )
      fail('INVALID_PROVIDER_RESPONSE', 502, attempts);
...
export async function executeProviderChain(
  ...
  for (const id of chain) {
    ...
    if (
      health &&
      health.failures >= config.ai.failureThreshold &&
      now() - health.lastFailureAt < config.ai.cooldownSeconds * 1000
    ) {
      attempts.push({ providerId: p.id, status: 0, outcome: 'circuit_open' });
      continue;
    }
    const url = endpoint(p, allowHosts);
    const key = await readSecret(p.secretRef);
    if (!key) fail('SECRET_MISSING', 503, attempts);
    for (let retry = 0; retry <= Math.min(p.retries, 3); retry++) {
      if (
        ...
        fail('AI_BUDGET_EXHAUSTED', 503, attempts);
      const controller = new AbortController();
      const timer = setTimeout(() => controller.abort(), Math.max(1, Math.min(p.timeoutMs, deadline - now())));
      const attempt = { providerId: p.id, model: p.model, protocol: p.protocol, status: 0, outcome: 'failed' };
      ...
      try {
        ...
        if (response.status >= 300 && response.status < 400) {
          await response.body?.cancel();
          fail('PROVIDER_REDIRECT', 502, attempts);
        }
        if (!response.ok) {
          await response.body?.cancel();
          if (response.status === 401) {
            attempt.outcome = 'authentication_failed';
            await healthStore?.recordFailure(p.id, now());
            break;
          }
          if (config.ai.retryStatuses.includes(response.status)) {
...
export function validateIntegration(kind, config) {
  const errors = [];
  if (kind === 'payos') {
    if (!config.clientId?.trim()) errors.push('CLIENT_ID_MISSING');
    for (const k of ['returnUrl', 'cancelUrl']) if (!safeUrl(config[k])) errors.push(`INVALID_${k}`);
  } else if (kind === 'zalo') {
    if (!/^\d+$/.test(config.appId || '')) errors.push('APP_ID_INVALID');
    for (const k of ['callbackUrl', 'returnUrl']) if (!safeUrl(config[k])) errors.push(`INVALID_${k}`);
...
function canonical(data) {
  ...
  return Object.keys(data)
    ...
    .map(k => {
      const v = data[k];
      if (v !== null && !['string', 'number', 'boolean'].includes(typeof v)) fail('INVALID_WEBHOOK', 400);
      return `${k}=${v === null || v === 'null' || v === 'undefined' ? '' : v}`;
    })
    .join('&');
}
export async function verifyPayosWebhook(payload, checksumKey) {
  ...
  if (
    !(await crypto.subtle.verify('HMAC', await hmacKey(checksumKey), bytes, new TextEncoder().encode(canonical(data))))
  )
  ...
  if (
    ...
    fail('INVALID_PAYMENT_EVENT', 400);
...
export async function createPaymentRequest(config, snapshot, { apiKey, checksumKey }, { fetchImpl = fetch } = {}) {
  ...
  if (
    !Number.isSafeInteger(snapshot.orderCode) ||
    snapshot.orderCode <= 0 ||
    !Number.isSafeInteger(snapshot.amountVnd) ||
    snapshot.amountVnd <= 0 ||
    typeof snapshot.description !== 'string' ||
    !/^[A-Za-z0-9 ]{1,9}$/.test(snapshot.description) ||
    !Number.isSafeInteger(snapshot.expiresAt) ||
    snapshot.expiresAt <= Math.floor(Date.now() / 1000) ||
    snapshot.expiresAt > 2147483647
  )
    fail('INVALID_ORDER_SNAPSHOT', 400);
  const body = {
    orderCode: snapshot.orderCode,
    amount: snapshot.amountVnd,
  ...
  try {
    const response = await fetchImpl('https://api-merchant.payos.vn/v2/payment-requests', {
      method: 'POST',
      headers: { 'x-client-id': config.clientId, 'x-api-key': apiKey, 'Content-Type': 'application/json' },
      body: JSON.stringify(body),
      redirect: 'manual',
      signal: controller.signal,
    });
    if (!response.ok) {
      await response.body?.cancel();
      fail('PAYOS_REQUEST_FAILED', 502);
    }
    const result = await readJson(response, 262144);
    if (result.code !== '00' || !result.data) fail('PAYOS_REQUEST_FAILED', 502);
    return result.data;
  } catch (e) {
    if (e instanceof RuntimeError) throw e;
    fail('PAYOS_NETWORK_ERROR', 502);
  } finally {
    clearTimeout(timer);
  }
}
      ...
      providers: [
        {
          ...provider,
          enabled: true,
          maxTokens: provider.protocol === 'responses' ? 512 : 32,
          retries: 0,
          timeoutMs: Math.min(provider.timeoutMs, 15000),
        },
      ],
      chain: [providerId],
      maxAttempts: 1,
