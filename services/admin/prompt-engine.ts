...
export function originalTasks(): Record<string, string> {
  ...
  for (const [key, module] of Object.entries({
    TUVI_TOPICS: 'tuvi',
    ZODIAC_TOPICS: 'zodiac',
    BATU_TOPICS: 'batu',
    NUMEROLOGY_TOPICS: 'numerology',
  })) {
    const topics = (
      originals.topics as Record<
        string,
        Array<{ id: string; prompt?: string; subs?: Array<{ id: string; prompt: string }> }>
      >
    )[key];
    for (const topic of topics)
      for (const sub of topic.subs || [topic])
        if (sub.prompt)
          out[
            module === 'batu' || module === 'numerology' ? `${module}--${sub.id}` : `${module}--${topic.id}--${sub.id}`
          ] = sub.prompt;
  }
  return out;
}
export function defaultPromptSettings() {
  return { templates: Object.fromEntries(templates.map(t => [t.id, t.template])), tasks: originalTasks() };
}
export function renderServicePrompt(
  node: PromptNode,
  serviceId: string,
  ...
  const visit = (n: PromptNode, depth = 0): PromptNode => {
    ...
    if (n.id === '$join')
      return { id: n.id, values: n.values.map(v => (typeof v === 'string' ? v : visit(v, depth + 1))) };
    const t = templates.find(t => t.id === n.id);
    if (
      !t ||
      (t.module !== module &&
        !t.id.includes('.profileContextText.') &&
        !(module === 'compat' && t.id === 'zodiac.compatPrompt.0'))
    )
      throw new Error('PROMPT_MODULE_MISMATCH');
    return {
      id: n.id,
...
function renderTask(id: string, rendered: string, template: string): string {
  const original = originalTasks()[id] || '';
  const names: string[] = [];
  const escaped = original.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const pattern = escaped.replace(/\\\{(SIGN|ELEMENT|RULER|NATAL)\\\}/g, (_, name) => {
    names.push(name);
    return '([\\s\\S]*?)';
  });
  const match = new RegExp('^' + pattern + '$').exec(rendered);
  const values = Object.fromEntries(names.map((n, i) => [n, match?.[i + 1] ?? '']));
