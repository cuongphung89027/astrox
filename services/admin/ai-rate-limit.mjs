...
const unavailable=()=>Response.json({code:'ai_safety_unavailable',error:'Dịch vụ đang kiểm tra giới hạn sử dụng. Vui lòng thử lại sau.'},{status:503,headers:{'cache-control':'no-store'}});
...
export async function limitAi(request,env,now=Date.now()){
 ...
 try{
  ...
  const rows=await env.DB.batch(windows.map(([bucket,max,expiry])=>env.DB.prepare('INSERT INTO ai_rate_limits(bucket,count,expires_at) VALUES(?,1,?) ON CONFLICT(bucket) DO UPDATE SET count=count+1 WHERE count<? RETURNING count').bind(bucket,expiry,max)));
  const denied=rows.findIndex(r=>!r.results?.length);
  if(denied<0)return null;
  return Response.json({code:'rate_limited',error:'Bạn đã đạt giới hạn lượt luận giải trong thời gian này. Vui lòng quay lại sau.'},{status:429,headers:{'cache-control':'no-store','retry-after':String(Math.max(1,Math.ceil((windows[denied][2]-now)/1000)))}});
