...
function rewardConfig(rewards) {
  return {
    ...
    milestones: (Array.isArray(rewards.milestones) ? rewards.milestones : []).map(m => ({
      day: Number(m.day) || 0,
      user: Number(m.user) || 0,
      inviter: Number(m.inviter) || 0,
    })),
  };
}
...
export async function settleRegistration(env, userId) {
  ...
  await env.DB.batch([
    env.DB.prepare(
      "INSERT OR IGNORE INTO user_referrals(user_id,inviter_id,created_at) SELECT ?,?,? WHERE ?!=? AND EXISTS(SELECT 1 FROM app_users WHERE id=? AND status='active') AND NOT EXISTS(WITH RECURSIVE ancestry(id) AS (SELECT inviter_id FROM user_referrals WHERE user_id=? UNION SELECT r.inviter_id FROM user_referrals r JOIN ancestry a ON r.user_id=a.id) SELECT 1 FROM ancestry WHERE id=?)",
    ).bind(userId, inviterId, now, userId, inviterId, inviterId, inviterId, userId),
    env.DB.prepare(
...
export async function recoverRegistrationRewards(env) {
  const rows = (
    await env.DB.prepare(
      "SELECT user_id FROM reward_events WHERE kind='registration' AND status='pending' ORDER BY created_at LIMIT 50",
    ).all()
  ).results;
  for (const row of rows)
    try {
      await settleRegistration(env, row.user_id);
    } catch {
      console.error(JSON.stringify({ event: 'rewards.registration_retry_failed' }));
    }
}
...
export async function firstTopupStatements(env, settings, order) {
  ...
  if (
    ...
    order.amount_vnd < (Number(rewards.firstTopupMinVnd) || 0)
  )
    return [];
...
export async function handleRewardsSummary(env, request) {
  ...
  let claimed = [];
  try {
    claimed = JSON.parse(att?.claimed_milestones || '[]');
  } catch {
    claimed = [];
  }
  return json(env, request, {
    ...
    ads: {
      enabled: adsConfigured(rewards) && !published?.config?.operations?.maintenance,
      points: Number(rewards?.ads?.points) || 0,
      dailyLimit: Number(rewards?.ads?.dailyLimit) || 0,
      used:
        (
          await env.DB.prepare(
            "SELECT COUNT(*) n FROM reward_ad_sessions WHERE user_id=? AND day=? AND status='granted'",
          )
            .bind(session.sub, day)
            .first()
        )?.n || 0,
      cooldownSeconds: Number(rewards?.ads?.cooldownSeconds) || 0,
    },
    attendance: {
...
export async function handleRewardsCheckin(env, request) {
  ...
  if (att) {
    try {
      previous = {
        lastDay: att.last_day,
        streak: att.streak,
        claimedMilestones: JSON.parse(att.claimed_milestones || '[]'),
      };
    } catch {
      /* state hỏng — coi như chưa từng điểm danh */
    }
  }
  let result;
  try {
    result = checkIn(previous, new Date(), rewards.__engine);
  } catch {
    return json(env, request, { error: 'attendance_invalid_state' }, 500);
  }
  if (result.duplicate)
    return json(env, request, { error: 'already_checked_in', day: result.day, streak: previous.streak }, 409);
  const now = nowIso(),
    token = crypto.randomUUID(),
    id = `attendance:${session.sub}:${result.day}`;
  const guard = { sql: 'EXISTS(SELECT 1 FROM reward_events WHERE id=? AND claim_token=?)', args: [id, token] };
  const statements = [
    env.DB.prepare(
      "INSERT OR IGNORE INTO reward_events(id,user_id,kind,status,claim_token,config_revision,payload,created_at,completed_at) SELECT ?,?,'attendance','completed',?,?,?,?,? WHERE COALESCE((SELECT last_day FROM user_attendance WHERE user_id=?),'')=? RETURNING id",
    ).bind(
      id,
      session.sub,
      token,
    ...
    env.DB.prepare(
      `INSERT INTO user_attendance(user_id,last_day,streak,claimed_milestones,updated_at) SELECT ?,?,?,?,? WHERE ${guard.sql} ON CONFLICT(user_id) DO UPDATE SET last_day=excluded.last_day,streak=excluded.streak,claimed_milestones=excluded.claimed_milestones,updated_at=excluded.updated_at`,
    ).bind(
      session.sub,
      result.day,
      result.state.streak,
      JSON.stringify(result.state.claimedMilestones),
      now,
      ...guard.args,
    ),
    ...creditStatements(
