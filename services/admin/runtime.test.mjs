...
test('reading service repairs only Han spans once with both attempts measured', async () => {
    ...
    options(async (u, o) => {
      ...
      return Response.json({
        choices: [
          {
            message: {
              content: calls === 1 ? '**Tử Vi**: 财星 ở cung Mệnh, năm 2026.' : '{"translations":["sao Tài"]}',
            },
            finish_reason: 'stop',
          },
        ],
  ...
  assert.equal(result.attempts[1].language, 'repaired');
  assert.equal(result.attempts[1].usage.input, 10);
  ...
  for (const fix of ['{"translations":["财星"]}', '{"translations":["sao Tài 2030"]}']) {
    ...
    await assert.rejects(
      executeProviderChain(
        ...
        options(async () => {
          return Response.json({
            choices: [{ message: { content: ++calls === 1 ? '财星' : fix }, finish_reason: 'stop' }],
          });
        }),
      ),
      e => e.code === 'READING_LANGUAGE_INVALID',
    );
    assert.equal(calls, 2);
  }
});
test('language repair respects configured total attempts and never bypasses a refusal', async () => {
  ...
  await assert.rejects(
    executeProviderChain(
      ...
      options(async () => {
        calls++;
        return Response.json({ choices: [{ message: { content: '财星' }, finish_reason: 'stop' }] });
      }),
    ),
    e => e.code === 'READING_LANGUAGE_INVALID',
  );
  ...
  await assert.rejects(
    executeProviderChain(
      ...
      options(async () =>
        ++calls === 1
          ? Response.json({ choices: [{ message: { content: '财星' }, finish_reason: 'stop' }] })
          : Response.json({ choices: [{ message: { refusal: 'No' }, finish_reason: 'stop' }] }),
      ),
    ),
...
test('health telemetry failure after repair cannot retry or repair the operation twice', async () => {
  ...
  let calls = 0,
    writes = 0;
  const result = await executeProviderChain(c, { ...input, serviceId: 'tuvi' }, async () => 'key', {
    ...options(async () =>
      Response.json({
        choices: [
          { message: { content: ++calls % 2 ? '财星' : '{"translations":["sao Tài"]}' }, finish_reason: 'stop' },
        ],
      }),
    ),
    healthStore: {
      ...
      recordFailure: async () => {},
      recordSuccess: async () => {
        if (++writes === 1) throw Error('telemetry unavailable');
      },
    },
  });
  assert.equal(result.choices[0].message.content, 'sao Tài');
  assert.equal(calls, 2);
