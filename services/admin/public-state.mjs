...
export function publicState(config, pathname, now = Date.now()) {
 ...
 const blocked=Boolean(module&&(config.maintenance||config.operations?.maintenance||['maintenance','hidden','draft'].includes(config.availability?.[module]||service?.status)));
