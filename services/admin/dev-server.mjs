...
const root = new URL('../../', import.meta.url),
  dir = pathToFileURL(settings.dataDir + '/');
mkdirSync(dir, { recursive: true, mode: 0o700 });
const credentials = new URL('credentials.json', dir);
if (!existsSync(credentials))
  writeFileSync(
    credentials,
    JSON.stringify(
      { password: randomBytes(24).toString('base64url'), encryptionKey: randomBytes(32).toString('base64') },
      null,
      2,
    ),
    { mode: 0o600 },
...
const credentialsValue = JSON.parse(readFileSync(credentials, 'utf8'));
const native = new DatabaseSync(fileURLToPath(new URL('admin.sqlite', dir)));
native.exec(readFileSync(new URL('migrations/admin.sql', root), 'utf8'));
...
if (process.env.ASTROX_LOCAL_BACKEND !== '0') {
  native.exec(
    readFileSync(new URL('services/backend/test/legacy-schema.sql', root), 'utf8')
...
for (const migration of ['ai-safety', 'service-unlocks', 'client-errors', 'feature-events', 'admin-insights', 'reward-events', 'bookings', 'promo-redemptions'])
  native.exec(readFileSync(new URL(`migrations/${migration}.sql`, root), 'utf8'));
const prepare = (query, args = []) => ({
  bind(...values) {
    return prepare(query, values);
  },
  async first() {
  ...
  execute() {
    const q = native.prepare(query);
    return q.columns().length ? { results: q.all(...args) } : { meta: { changes: q.run(...args).changes } };
  },
});
const DB = {
  prepare,
...
const env = {
  DB,
  PROVIDER_ALLOWED_HOSTS: process.env.PROVIDER_ALLOWED_HOSTS || '',
  LOCAL_ADMIN: true,
  LOCAL_ADMIN_PASSWORD: credentialsValue.password,
  ADMIN_ENCRYPTION_KEY: credentialsValue.encryptionKey,
};
...
createServer(async (req, res) => {
  try {
    ...
    let request = new Request(`http://${host}${req.url}`, {
      method: req.method,
      headers,
    ...
    const origin = headers.get('origin'),
      rewritten = rewriteLocalOrigin(origin, host, trustedPreview);
    if (rewritten !== origin) {
      headers.set('origin', rewritten);
    ...
    if (trustedPreview && origin === trustedPreview && response.headers.has('set-cookie'))
      response.headers.set('set-cookie', response.headers.get('set-cookie') + '; Secure');
    res.writeHead(response.status, Object.fromEntries(response.headers));
    res.end(Buffer.from(await response.arrayBuffer()));
  } catch {
    res.writeHead(500, { 'content-type': 'application/json' });
    res.end(JSON.stringify({ error: 'Local admin server error' }));
  }
}).listen(settings.port, '127.0.0.1', () =>
  console.log(
    `Admin API: http://127.0.0.1:${settings.port} — bootstrap credentials in ${settings.dataDir}/credentials.json`,
  ),
);
