...
export async function verifyZaloUser(token,fetchImpl=fetch){
 if(typeof token!=='string'||!token)throw new Error('zalo_identity_unverified');
 const r=await fetchImpl('https://graph.zalo.me/v2.0/me?fields=id,name,picture',{headers:{access_token:token},redirect:'error',signal:AbortSignal.timeout(15000)});const me=await r.json().catch(()=>null);
 if(!r.ok||!me?.id||me.error)throw new Error('zalo_identity_unverified');return me;
}
...
export async function zaloCallback(env,request,settings,fetchImpl=fetch){
 ...
 if(!state||!code||!cookie||!await equal(state,cookie))return json(env,request,{error:'invalid_oauth_state'},400);
 const row=await env.DB.prepare("DELETE FROM oauth_states WHERE id=? AND julianday(created_at)>julianday('now','-10 minutes') RETURNING code_verifier").bind(state).first();if(!row)return json(env,request,{error:'invalid_or_expired_state'},400);
 const r=await fetchImpl('https://oauth.zaloapp.com/v4/access_token',{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded',secret_key:env.ZALO_APP_SECRET},body:new URLSearchParams({code,app_id:env.ZALO_APP_ID,grant_type:'authorization_code',code_verifier:row.code_verifier}),redirect:'error',signal:AbortSignal.timeout(15000)});
 const tokens=await r.json().catch(()=>null);if(!r.ok||!tokens?.access_token)return json(env,request,{error:'token_exchange_failed'},502);
 let me;try{me=await verifyZaloUser(tokens.access_token,fetchImpl);}catch{return json(env,request,{error:'zalo_identity_unverified',message:'Chưa xác minh được danh tính từ Zalo. Vui lòng thử lại.'},502);}
