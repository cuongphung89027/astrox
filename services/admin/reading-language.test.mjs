...
import {inspectReading,applyTranslations,languageRepairMessages,translateKnownTerms,hasHan,LANGUAGE_POLICY_VERSION} from './reading-language.ts';
...
test('repair changes only exact foreign spans; preserves named facts, numbers and Markdown',()=>{
 ...
 assert.equal(applyTranslations(plan,JSON.stringify({translations:['sao Tài']})),'**Tử Vi** ở cung Mệnh năm 2026: sao Tài và The Fool. sao Tài xuất hiện 2 lần.');
});
test('structured results retain keys, number values, nesting and escaped characters',()=>{
 const source=JSON.stringify({percent:42,strengths:['财星'],watchouts:[],advice:'Giữ "The Fool"',meta:{ok:true,x:null}});
 const result=JSON.parse(applyTranslations(inspectReading(source),'{"translations":["sao Tài"]}'));
 assert.deepEqual(result,{percent:42,strengths:['sao Tài'],watchouts:[],advice:'Giữ "The Fool"',meta:{ok:true,x:null}});
});
test('foreign JSON keys fail closed instead of silently changing schema',()=>assert.throws(()=>inspectReading('{"财星":"abc"}'),/READING_LANGUAGE_INVALID/));
test('bounded repairs reject extra fields, added facts, residual Han and wrong number of translations',()=>{
 ...
 for(const value of [{translations:[]},{translations:['sao Tài','thêm']},{translations:['财星']},{translations:['sao Tài 99']},{translations:['sao Tài'],extra:true},{translations:['']},{translations:['\n# sai']}])assert.throws(()=>applyTranslations(plan,JSON.stringify(value)),/READING_LANGUAGE_INVALID/);
});
...
test('composed known can chi and star facts cannot be renamed by a translation model',()=>{
 assert.equal(applyTranslations(inspectReading('Nhật trụ 甲子.'),'{"translations":["Bính Ngọ"]}'),'Nhật trụ Giáp Tý.');
});
test('JSON repair preserves original numeric lexemes including integers beyond JS precision',()=>{
 ...
 assert.equal(applyTranslations(inspectReading(original),'{"translations":["sao Tài"]}'),'{"id":9007199254740993,"rate":1.00,"advice":"sao Tài"}');
});
