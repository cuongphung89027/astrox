...
export function normalizeUsage(raw: unknown, protocol: string): Usage {
  const u = object(raw),
    input = count(u.input_tokens ?? u.prompt_tokens),
    output = count(u.output_tokens ?? u.completion_tokens);
  ...
  return {
    input: protocol === 'anthropic' && input !== null ? input + (cacheRead ?? 0) + (cacheWrite ?? 0) : input,
    output,
    cacheRead,
    cacheWrite,
  };
...
export function estimateCost(u: Usage, p?: Pricing): number | null {
  if (
    ...
    u.cacheRead === null ||
    u.cacheWrite === null ||
    u.cacheRead + u.cacheWrite > u.input ||
    Object.values(p).some(n => !Number.isFinite(n) || n < 0)
  )
    return null;
  return (
    ((u.input - u.cacheRead - u.cacheWrite) * p.input +
      u.output * p.output +
...
export function parseAttempts(raw: unknown): Attempt[] {
  ...
  return list
    .map(object)
    .filter(a => typeof a.providerId === 'string')
    .map(a => {
      ...
      return {
        ...
        status: count(a.status) || 0,
        durationMs: count(a.durationMs),
        usage: {
          input: count(u.input),
        ...
        costUsd: typeof a.costUsd === 'number' && Number.isFinite(a.costUsd) && a.costUsd >= 0 ? a.costUsd : null,
      };
    });
}
...
export function createAiSummary(
  ...
  function add(rows: MetricRow[]) {
    for (const row of rows) {
      if (filters.service && row.service_id !== filters.service) continue;
      const all = parseAttempts(row.attempts),
        selected = all.filter(
          a =>
            (!filters.provider || a.providerId.split(':')[0] === filters.provider) &&
            (!filters.model || a.model === filters.model),
        );
      if ((filters.provider || filters.model) && !selected.length) continue;
      requests++;
      const ok = row.status === 'success' || row.status === 'replayed',
        isBlocked = blockedStatuses.has(row.status);
      if (ok) {
        success++;
        if (row.status === 'replayed') replayed++;
      } else if (isBlocked) {
        blocked++;
      } else {
        failed++;
        if (providerStatuses.has(row.status)) providerFailures++;
        else serviceFailures++;
        const code = errorCodes.has(row.status) || errorCodes.size < 100 ? row.status : 'other';
      ...
      const date = row.created_at.slice(0, 10),
        day = daily.get(date) || { day: date, requests: 0, failed: 0 };
      day.requests++;
      if (!ok && !isBlocked) day.failed++;
      daily.set(date, day);
      const upstream = selected.filter(
        a => Boolean(a.providerId) && !['circuit_open', 'delegated', 'refunded', 'refund_pending'].includes(a.outcome),
      );
      const fullUpstream = all.filter(
        a => Boolean(a.providerId) && !['circuit_open', 'delegated', 'refunded', 'refund_pending'].includes(a.outcome),
      );
      ...
      for (const a of upstream) {
        ...
        const rawKey = `${a.providerId}|${a.model}`,
          key = groups.has(rawKey) || groups.size < 500 ? rawKey : 'other',
          g = groups.get(key) || {
            provider: key === 'other' ? 'other' : a.providerId.split(':')[0],
            route: key === 'other' ? 'other' : a.providerId,
            model: key === 'other' ? 'Khác' : a.model || 'Chưa ghi model',
            attempts: 0,
            failed: 0,
            input: null,
            output: null,
        ...
        g.attempts++;
        if (a.outcome !== 'success') g.failed++;
        for (const key of ['input', 'output', 'cacheRead', 'cacheWrite'] as const)
          if (a.usage[key] !== null) {
            tokens[key] = (tokens[key] ?? 0) + a.usage[key]!;
            hasUsage = true;
          }
        for (const key of ['input', 'output'] as const)
          if (a.usage[key] !== null) g[key] = (g[key] ?? 0) + a.usage[key]!;
        if (a.usage.input !== null && a.usage.output !== null) {
          totalTokens = (totalTokens ?? 0) + a.usage.input + a.usage.output;
          completeUsageAttempts++;
        }
        if (a.usage.cacheRead !== null) {
          cacheKnownAttempts++;
          if (a.usage.cacheRead > 0) cacheReadAttempts++;
        }
        if (a.costUsd !== null) {
          costUsd = (costUsd ?? 0) + a.costUsd;
          pricedAttempts++;
        }
        groups.set(key, g);
      }
  ...
  function finish() {
    ...
    return {
      ...
      serviceFailures,
      providerFailures,
      completedRequests: success + failed,
      errorRate: success + failed ? (failed / (success + failed)) * 100 : null,
      attempts,
      attemptErrors,
      attemptErrorRate: attempts ? (attemptErrors / attempts) * 100 : null,
      retries,
      fallbackRequests,
      usageRequests,
      tokens,
      ...
      cacheReadAttempts,
      cacheKnownAttempts,
      cacheHitRate: cacheKnownAttempts ? (cacheReadAttempts / cacheKnownAttempts) * 100 : null,
      costUsd,
      pricedAttempts,
      delegated,
