...
test('registration awards both parties; rejects unverified, existing and self referrals', () => {
  ...
  assert.deepEqual(
    registrationRewards(args).map(x => x.points),
    [5, 5],
  );
  for (const bad of [{ verified: false }, { isNew: false }, { inviterId: 'new' }])
    assert.deepEqual(registrationRewards({ ...args, ...bad }), []);
});
test('first paid settlement awards inviter only, once', () => {
  ...
  for (const bad of [{ settled: false }, { paidAmount: 0 }, { alreadyRewarded: true }, { inviterId: 'new' }])
    assert.deepEqual(firstTopupReward({ ...args, ...bad }), []);
});
test('daily duplicate is no-op; Vietnam midnight advances day', () => {
  ...
  assert.equal(a.state.lastDay, '2026-09-21');
...
test('10 consecutive days yield 38 check-in points and 10 inviter points', () => {
  let s = fresh,
    ...
    inviter = 0;
  for (let d = 1; d <= 10; d++) {
    const r = checkIn(s, at(String(d).padStart(2, '0')));
    s = r.state;
    total += r.points;
    inviter += r.inviterPoints;
  }
  assert.equal(total, 38);
  assert.equal(inviter, 10);
  assert.deepEqual(s.claimedMilestones, [3, 7, 10]);
  const next = checkIn(s, at('11'));
  assert.equal(next.points, 2);
  assert.equal(next.inviterPoints, 0);
});
...
test('admin config rejects invalid, duplicate or unbounded point values', () => {
  ...
  assert.throws(() =>
    validateConfig({
      ...DEFAULT_REWARDS,
      milestones: [
        { day: 3, user: 3, inviter: 2 },
        { day: 3, user: 5, inviter: 3 },
      ],
    }),
  );
});
