...
export async function accountData(env, request, userId) {
  ...
  try {
    ...
    return reply({ error: 'invalid_saved_data' }, 503);
  }
  ...
  let b;
  try {
    b = await bodyJson(request, 1800000);
  } catch {
    return reply({ error: 'data_too_large' }, 413);
  }
  if (!Number.isSafeInteger(b?.expectedRevision) || b.expectedRevision < 0)
    return reply({ error: 'revision_required' }, 428);
  if (b.expectedRevision !== revision) return reply({ error: 'sync_conflict' }, 409);
  const next = b.payload;
  if (!next || typeof next !== 'object' || Array.isArray(next) || Object.keys(next).some(k => !FIELDS.includes(k)))
    return reply({ error: 'invalid_data' }, 400);
  const raw = JSON.stringify(preserveUserData(payload, next));
  if (new TextEncoder().encode(raw).length > 1700000) return reply({ error: 'data_too_large' }, 413);
  if (row && raw === row.payload) return reply({ ok: true, revision });
  const updated = Math.max(Date.now(), revision + 1);
  const changed = await env.DB.prepare(
    'INSERT INTO user_data(user_id,payload,updated_at) VALUES(?,?,?) ON CONFLICT(user_id) DO UPDATE SET payload=excluded.payload,updated_at=excluded.updated_at WHERE user_data.updated_at=? RETURNING updated_at',
  )
    .bind(userId, raw, updated, revision)
    .first();
