...
export async function onRequestPost(context) {
  ...
  const config={ai:{enabled:true,systemPrompt:'',chain:['legacy'],providers:[{id:'legacy',name:'Legacy',baseUrl:ENDPOINT.replace(/\/responses$/,''),protocol:'responses',model:DEFAULT_MODEL,enabled:true,secretRef:'legacy',timeoutMs:90000,retries:1,maxTokens:Math.min(Number(payload.max_tokens)||4000,16000),temperature:typeof payload.temperature==='number'?payload.temperature:0.7}],totalTimeoutMs:90000,maxAttempts:3,cooldownSeconds:0,failureThreshold:3,retryStatuses:[...RETRYABLE_STATUS]},billing:{services:[{id:'legacy',module:'legacy',prompt:'',chain:[]}]}};
  try{
    const result=await executeProviderChain(config,{serviceId:'legacy',messages:input},async()=>key,{allowHosts:[new URL(ENDPOINT).hostname]});
    return json(200,{choices:result.choices,model:result.model,usage:result.usage,languagePolicyVersion:result.languagePolicyVersion});
  }catch(error){
    return json(error?.status||502,{error:error?.code==='READING_LANGUAGE_INVALID'?'Luận giải chưa đạt yêu cầu tiếng Việt. Vui lòng thử lại.':'Không nhận được luận giải hợp lệ. Vui lòng thử lại.'});
  }
}
