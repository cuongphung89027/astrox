...
export const json = (env, request, data, status = 200) =>
  Response.json(data, {
    status,
    headers: { ...corsHeaders(env, request), 'cache-control': 'no-store', 'x-content-type-options': 'nosniff' },
  });
export function trustedOrigin(env, request) {
  const origin = request.headers.get('Origin');
  return origin === (env.APP_ORIGIN || 'https://theastrox.space') || origin === new URL(request.url).origin;
}
