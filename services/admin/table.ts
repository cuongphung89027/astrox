...
export function selectRows(
  rows: DataRow[],
  options: { query?: string; status?: string; sortKey?: string; direction?: 'asc' | 'desc' } = {},
): DataRow[] {
  const query = searchable(options.query?.trim() || '');
  const selected = rows.filter(
    row =>
      (!options.status || cellText(row.status) === options.status) &&
      (!query || searchable(Object.values(row).map(cellText).join(' ')).includes(query)),
  );
  if (!options.sortKey) return selected;
  const key = options.sortKey,
    sign = options.direction === 'desc' ? -1 : 1;
  return selected.sort(
    (a, b) =>
      sign *
      (typeof a[key] === 'number' && typeof b[key] === 'number'
        ? (a[key] as number) - (b[key] as number)
        : cellText(a[key]).localeCompare(cellText(b[key]), 'vi', { numeric: true })),
  );
}
export function pageRows(rows: DataRow[], requestedPage = 1, requestedSize = 20) {
  ...
  return {
    ...
    end: Math.min(rows.length, offset + size),
  };
}
export function rowsCsv(rows: DataRow[], labels: Record<string, string> = {}) {
  ...
  return (
    '\uFEFF' +
    [keys.map(k => escape(labels[k] || k)).join(','), ...rows.map(row => keys.map(k => escape(row[k])).join(','))].join(
      '\r\n',
    )
  );
}
