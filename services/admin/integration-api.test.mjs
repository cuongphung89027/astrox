...
test('paid reading language repair completes one charge; failed repair refunds without complete', async () => {
  ...
  for (const bad of [false, true]) {
    ...
    c.ai.providers = [
      {
        ...
        maxTokens: 100,
        temperature: 0.5,
        secretRef: 'provider:language',
      },
    ];
    await saveSecret(env, 'owner', 'provider:language', 'test-key');
    await publish(env, 'owner', c, 0, 'test');
    const operations = [];
    let saved;
    env.ASTROX_BACKEND = {
      fetch: async r => {
        const p = new URL(r.url).pathname.split('/').at(-1);
        operations.push(p);
        if (p === 'charge') return Response.json({ ok: true, chargeId: 'charge-language', points: 10 });
        if (p === 'complete') saved = await r.json();
    ...
    globalThis.fetch = async () =>
      Response.json({
        choices: [
          {
            message: {
              content: ++calls === 1 ? '财星 năm 2026' : JSON.stringify({ translations: [bad ? '财星' : 'sao Tài'] }),
            },
            finish_reason: 'stop',
          },
        ],
    ...
    try {
      const response = await handleConfiguredAi(
        request('/api/ai', {
          operationId: 'language-operation',
          expectedPoints: 10,
          serviceId: 'tuvi',
          messages: [{ role: 'user', content: 'Luận giải' }],
        }),
        env,
      );
