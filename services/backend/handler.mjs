...
export async function moduleAccess(env, request) {
  ...
  if (session) {
    const rows = await env.DB.prepare('SELECT module,enabled FROM user_module_access WHERE user_id=?')
      .bind(session.sub)
      .all();
    for (const r of rows.results) access[r.module] = Boolean(access[r.module] && r.enabled);
  }
  ...
  try {
    ...
    if (path === '/api/health')
      return json(env, request, {
        ...
        version: '2026-09-24-point-expiry-1',
      });
    ...
    if (path === '/auth/zalo/callback' && method === 'GET') {
      const s = await runtimeSettings(env);
      return await zaloCallback(s.env, request, s);
    }
    if (path === '/auth/zalo/finish') {
      const s = await runtimeSettings(env);
      return await zaloFinish(s.env, request, s);
    }
    if (path === '/auth/zalo/complete')
      return json(
        env,
        request,
    ...
    if (path === '/api/user-data') {
      ...
      const r = await accountData(env, request, `zalo:${session.sub}`);
    ...
    if (path === '/api/topup/packages' && method === 'GET') {
      const s = await runtimeSettings(env);
      return json(env, request, { packages: s.packages, revision: s.revision });
    }
    ...
    if (path === '/api/topup/history' && method === 'GET') {
      ...
      const rows = await env.DB.prepare(
        'SELECT order_code,amount_vnd,points,status,created_at,paid_at FROM topup_orders_zalo WHERE user_id=? ORDER BY created_at DESC LIMIT 50',
      )
        .bind(session.sub)
        .all();
      return json(env, request, { orders: rows.results });
    }
    ...
    if (path.startsWith('/api/rewards/ads/'))
      return await handleRewardedAds(env, request, path.slice('/api/rewards/ads/'.length));
    if (path === '/api/rewards/checkin' && method === 'POST') return await handleRewardsCheckin(env, request);
    if (path === '/zalo_verifierHiMyTOFJ571A-hHxZ_WyNqxxiMEYZMiqDZSq.html')
      return new Response(
        '<!doctype html><meta property="zalo-platform-site-verification" content="HiMyTOFJ571A-hHxZ_WyNqxxiMEYZMiqDZSq">',
    ...
    console.error(
      JSON.stringify({
        ...
        code: e.message === 'request_too_large' ? 'request_too_large' : 'internal_error',
      }),
    );
    return json(
      env,
      request,
      { error: e.message === 'request_too_large' ? 'request_too_large' : 'backend_unavailable' },
      e.message === 'request_too_large' ? 413 : 503,
    );
  }
}
export async function internalFetch(request, env) {
  ...
  if (path === '/internal/admin/users/status' && request.method === 'POST')
    return await internalUserStatus(env, request);
  if (path === '/internal/admin/wallet/adjust' && request.method === 'POST')
    return await internalWalletAdjust(env, request);
  if (path === '/internal/admin/login-diagnostics' && request.method === 'GET')
    return Response.json({
      rows: (
        await env.DB.prepare(
          'SELECT id,stage,detail,created_at FROM login_diagnostics ORDER BY id DESC LIMIT 200',
        ).all()
      ).results,
      source: 'astrox-api',
      readOnly: true,
  ...
  const queries = {
    users:
      'SELECT id,display_name,email,status,created_at,updated_at FROM app_users ORDER BY created_at DESC LIMIT 200',
    wallet:
      'SELECT a.user_id,u.display_name,a.balance,a.updated_at FROM zalo_point_accounts a JOIN app_users u ON u.id=a.user_id ORDER BY a.updated_at DESC LIMIT 200',
    reports:
      'SELECT id,user_id,order_code,amount_vnd,points,status,created_at,paid_at FROM topup_orders_zalo ORDER BY created_at DESC LIMIT 200',
    rewards:
  ...
  if (queries[kind])
    return Response.json({
      rows: (await env.DB.prepare(queries[kind]).all()).results,
      source: 'astrox-api',
      readOnly: true,
    });
...
async function internalUserStatus(env, request) {
  ...
  const r = await env.DB.prepare('UPDATE app_users SET status=?,updated_at=? WHERE id=?')
...
async function internalWalletAdjust(env, request) {
  ...
  const batch = await env.DB.batch([
    ...
    env.DB.prepare(
      'INSERT INTO zalo_point_ledger(id,user_id,delta,reason,reference_id,created_at) VALUES(?,?,?,?,?,?)',
    ).bind(crypto.randomUUID(), userId, delta, 'admin_adjust', referenceId, now),
    env.DB.prepare('UPDATE zalo_point_accounts SET balance=balance+?,updated_at=? WHERE user_id=?').bind(
      delta,
      now,
      userId,
    ),
