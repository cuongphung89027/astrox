...
const services = new Map([...MODULE_IDS.map(m => [m, m]), ...SERVICE_CATALOG.map(s => [s.id, s.module])]);
...
export async function featureEvent(request, env, now = Date.now()) {
  ...
  try {
    ...
    const limits = await env.DB.batch(
      [
        [`ip:${hour}:${ip}`, 120, (hour + 1) * 3600000],
        [`global:${day}`, 100000, (day + 1) * 86400000],
      ].map(([key, max, expiry]) =>
        env.DB.prepare(
          'INSERT INTO feature_event_limits(bucket,count,expires_at) VALUES(?,1,?) ON CONFLICT(bucket) DO UPDATE SET count=count+1 WHERE count<? RETURNING count',
        ).bind(key, expiry, max),
      ),
    );
    if (limits.some(r => !r.results?.length)) return reply(429);
    await env.DB.batch([
      env.DB.prepare('DELETE FROM feature_events WHERE created_at < ?').bind(
        new Date(now - 90 * 86400000).toISOString(),
      ),
      env.DB.prepare('DELETE FROM feature_event_limits WHERE expires_at <= ?').bind(now),
      env.DB.prepare(
        'INSERT OR IGNORE INTO feature_events(id,event,module,service_id,source,session_id,device,created_at) VALUES(?,?,?,?,?,?,?,?)',
      ).bind(...keys.map(k => body[k]), new Date(now).toISOString()),
    ]);
    return reply(204);
  } catch {
    return reply(503);
