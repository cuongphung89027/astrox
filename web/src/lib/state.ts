...
export async function refreshPromptRevision(){try{const r=await fetch('/api/site-config',{cache:'no-store'});if(!r.ok){activePromptRevision=null;return;}const d=await r.json();setPromptRevision(d.revision??0);}catch{activePromptRevision=null;}}
...
export function readAiCache(group: CacheGroup, key: string, force = false): string {
  ...
  if(activePromptRevision===null || (entry as {configRevision?:number}|undefined)?.configRevision!==activePromptRevision)return '';
