...
test('wallet HTML gets a fresh matching CSP nonce and cannot be cached across users',async()=>{
 ...
 globalThis.HTMLRewriter=class{on(selector,handler){assert.equal(selector,'script');this.handler=handler;return this;}transform(response){this.handler.element({setAttribute(name,value){assert.equal(name,'nonce');nonces.push(value);}});return response;}};
 try{for(let i=0;i<2;i++){const response=await onRequest({request:new Request('https://theastrox.space/hoso?section=points'),env:{ASSETS:{fetch:async()=>new Response('<script>safe()</script>',{headers:{'content-type':'text/html','etag':'old'}})}}});assert.ok(response.headers.get('content-security-policy').includes(`'nonce-${nonces[i]}' 'strict-dynamic'`));assert.equal(response.headers.get('cache-control'),'private, no-store');assert.equal(response.headers.has('etag'),false);}assert.notEqual(nonces[0],nonces[1]);}finally{globalThis.HTMLRewriter=previous;}
});
