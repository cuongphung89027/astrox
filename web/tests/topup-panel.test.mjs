...
async function fixture({user={id:'alice'},create,promoCheck,redeemPromo}={}) {
  ...
  const api={loadTopupPackages:async()=>[{amount_vnd:10000,points:100},{amount_vnd:20000,points:230}],loadTopupHistory:async()=>[],promoCheck:promoCheck??(async()=>({ok:true,bonus:25})),redeemPromo:redeemPromo??(async()=>({ok:true,points:25})),createTopup:async(...args)=>{calls.push(args);return create?create(...args):{error:'payos'};}};
...
test('direct Point promo credits the wallet and tells the user after redemption',async()=>{
  const f=await fixture({promoCheck:async()=>({ok:true,kind:'direct_points',bonus:25})});await f.load();
  const before=f.getRefreshes();f.promoInput().onChange({target:{value:'GIFT'}});f.render();f.apply().onClick();await tick();await tick();f.render();
  assert.match(f.pick('p',p=>p.role==='status'&&String(p.children).includes('Áp dụng thành công')).children,/đã cộng 25 Point/);
  assert.ok(f.getRefreshes()>before);
  assert.equal(f.pay().disabled,true);
});
test('promo check shows the exact minimum topup requirement',async()=>{
  const f=await fixture({promoCheck:async()=>({ok:false,error:'promo_min_amount',minAmountVnd:50000})});await f.load();f.select(10000);
  f.promoInput().onChange({target:{value:'MIN50'}});f.render();f.apply().onClick();await tick();f.render();
  assert.match(f.pick('p',p=>p.role==='status'&&String(p.children).includes('tối thiểu')).children,/50\.000 ₫/);
});
