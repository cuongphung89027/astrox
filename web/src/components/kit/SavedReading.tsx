...
export function ReadingInvitation({label, onRun, serviceId, prompt}: {label:string; onRun:()=>void; serviceId?:string; prompt?:string}) {
  ...
  return <div className={styles.invitation}><div><span>CHƯA CÓ KẾT QUẢ</span><p>Khám phá khi bạn sẵn sàng</p></div><button onClick={onRun} disabled={!!serviceId && price.pending}>{label}{serviceId && price.paid && ` · ${price.text}`}<span aria-hidden="true">↗</span></button></div>;
}
