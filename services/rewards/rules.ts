...
export function validateConfig(c: RewardConfig): void {
  ...
  for (const m of c.milestones) {
    ...
    days.add(m.day);
    amounts.push(m.user, m.inviter);
  }
  if (amounts.some(n => !Number.isSafeInteger(n) || n < 0) || !Number.isSafeInteger(amounts.reduce((a, b) => a + b, 0)))
...
export function registrationRewards(
  r: Referral & { verified: boolean; isNew: boolean },
  c = DEFAULT_REWARDS,
): Reward[] {
  validateConfig(c);
...
export function firstTopupReward(
  r: Referral & { settled: boolean; paidAmount: number; alreadyRewarded: boolean },
  c = DEFAULT_REWARDS,
): Reward[] {
  validateConfig(c);
  if (!validReferral(r) || !r.settled || !Number.isSafeInteger(r.paidAmount) || r.paidAmount <= 0 || r.alreadyRewarded)
    return [];
...
export function checkIn(previous: Attendance, now: Date, c = DEFAULT_REWARDS) {
  ...
  if (previous.lastDay !== null) {
    ...
    if (
      !/^\d{4}-\d{2}-\d{2}$/.test(previous.lastDay) ||
      !Number.isFinite(timestamp) ||
      new Date(timestamp).toISOString().slice(0, 10) !== previous.lastDay
    )
  ...
  if (gap === 0)
    return { day, points: 0, inviterPoints: 0, milestones: [] as number[], duplicate: true, state: previous };
  ...
  return {
    day,
    points: c.daily + earned.reduce((n, m) => n + m.user, 0),
    inviterPoints: earned.reduce((n, m) => n + m.inviter, 0),
    milestones: earned.map(m => m.day),
    duplicate: false,
