...
try{
 for(const width of [390,1440]){
  ...
  assert.equal(await p.locator('#kd-method option').count(),6);assert.ok(await p.getByRole('button',{name:'Lập quẻ',exact:false}).isDisabled());
  await checkpoint(p,`${width} coin input`);
  for(let i=1;i<=6;i++)await p.getByRole('button',{name:`Gieo lần ${i}`,exact:false}).click();
  await p.getByRole('button',{name:'Lập quẻ',exact:false}).click();await p.getByRole('heading',{name:'Quẻ của bạn'}).waitFor();await checkpoint(p,`${width} six coin result`);
